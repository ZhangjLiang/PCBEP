...
Center = T.Center()
#Normalscale = T.NormalizeScale()
#   triangulation get face
Delaunay = T.Delaunay()
#   Calculate the normal vector based on face
Normal = T.GenerateMeshNormals()
...
class Net(torch.nn.Module):
    ...
    def forward(self, data, use=None):
        ...
        out_pcbep = out.unsqueeze(0)
        esm_list_2 = esm_list.unsqueeze(0)
        out_esm = self.crossattention(out_pcbep, esm_list_2)
        out = self.mlp_esm(out_esm).squeeze(0)
        ...
        # out = (out_esm[:, 1].unsqueeze() + out_only[:, 1].unsqueeze(1))
...
for fold in [0, 1, 2, 3, 4]:
    ...
    n_epochs = 15000
    patience = 20
    checkpoint = 'checkpoint/724_esm_CSattention_' + str(fold) + '.pt'
...
for i in range(5):
    ...
    with open('result/724_CSattention_(' + str(i) + ').txt', 'w') as f:
...
for i in range(5):
    ...
    out_total = out_total.tolist()
    aa_total = aa_total.tolist()
    with open('result/272_CSattention_(' + str(i) + ').txt', 'w') as f:
        for i in range(len(out_total)):
            f.write(str(aa_total[i]) + '\t' + str(out_total[i]) + '\n')
