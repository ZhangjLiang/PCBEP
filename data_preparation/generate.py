        ...
        with open(options.output, 'w') as nfe:
            with open('test_data_feature_surface.txt', 'r') as fe:
                ...
                for i in range(int(num_acid)):
                    ...
                    with open(options.pssm + name.upper() + '.pssm', 'r') as ps:
                        ps.readline()
                        ps.readline()
                        ps.readline()
...
def start(options):
    with open('test_data_feature_surface.txt', 'w') as ff:
        ...
        with open(label_file, 'r') as fp, open(surface_file) as fp3:
            ...
            for line1 in fp:  # line1:str '>3jcx-A\n'
                if line1.startswith('>'):
                    ...
                    ##seq = row1[1]
                    seq = row1[0][6]
                    file = options.pdb + file_name.upper() + '-' + seq.upper() + '.pdb'
...
def main():
    ...
    parser = OptionParser()
    parser.add_option("-l", "--label", type="string", dest="label", default='/home/yan/bcell/PCBEP_all_code/data/label/label.txt',
                      help="Label file path entered.")
    parser.add_option("-s", "--surface", type="string", dest="surface", default='/home/yan/bcell/PCBEP_all_code/data/surface/surface_total.txt',
                      help="Surface file path entered.")
    parser.add_option("-p", "--pdb", type="string", dest="pdb", default='/home/yan/bcell/PCBEP_all_code/data/pdb_sasa/',
                      help="Checkpoint folder path entered.")
    parser.add_option("-m", "--pssm", type="string", dest="pssm", default='/home/yan/bcell/PCBEP_all_code/data/pssm/',
                      help="Checkpoint folder path entered.")
    parser.add_option("-o", "--output", type="string", dest="output", default='/home/yan/bcell/PCBEP_all_code/68_data_feature_surface.txt',
                      help="Output the results to a file.")
    (options, args) = parser.parse_args()
    # options = {'label': path + '/data/label/label.txt', 'surface': path + '/data/surface/surface_total.txt', 'pdb': path + '/data/fasta/pdb_sasa/',
    #            'pssm': path + '/data/pssm/', 'output': path + 'data_feature_surface.txt', 'path': path}
    start(options)


if __name__ == '__main__':
    parser = OptionParser()
    parser.add_option("-l", "--label", type="string", dest="label", default='64_indep_label.txt',
                      help="Label file path entered.")
    parser.add_option("-s", "--surface", type="string", dest="surface", default='64_surface_indep.txt',
                      help="Surface file path entered.")
    parser.add_option("-p", "--pdb", type="string", dest="pdb", default='pdb_chain/all/',
                      help="Checkpoint folder path entered.")
    parser.add_option("-m", "--pssm", type="string", dest="pssm", default='pssm/',
                      help="Checkpoint folder path entered.")
    parser.add_option("-o", "--output", type="string", dest="output", default="data_feature_surface.txt",
                      help="Output the results to a file.")
    (options, args) = parser.parse_args()
    start(options)
